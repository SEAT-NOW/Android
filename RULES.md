...
## 📝 Rule 7. 리팩토링 및 변경 사항 문서화와 Github 커밋 자동화
AI는 코드 리팩토링이나 주요 수정 작업이 완료될 때마다 사용자(원장님)가 변경 내용을 쉽게 파악할 수 있도록 **반드시 Markdown 기반의 Artifact(문서)**를 생성하여 제공해야 한다.
또한, 리팩토링 결과물에 대한 문서 제공과 동시에 백그라운드 터미널 커맨드를 통해 `git add .` 및 `git commit -m "..."`을 **즉시 자동화하여 수행**해야 한다.

* **[DO]** 리팩토링 완료 직후 변경사항에 대한 Artifact(.md) 파일 생성 및 제공
* **[DO]** 사용자에게 물어보지 않고 곧바로 `git commit` 자동 수행

### 📌 커밋 메시지 형식 (기존 Git 히스토리 기반)
커밋 메시지는 반드시 아래 형식을 준수한다:
...
| TYPE | 사용 상황 |
|------|----------|
| `FEAT` | 새로운 기능 추가 및 주요 로직이 변경/추가되는 버그 해결 시 |
| `REF` | 단순 UI 개선, 코드 구조 리팩토링 및 단순 오류 수정 (※ `FIX` 사용 엄격히 금지) |
| `DOCS` | 문서 작업 (RULES.md, README 등) |
| `CHORE` | 빌드, 설정, 의존성 등 기타 변경 |

**예시:**
```
(REF) owner/signup UseCase 분리 및 Mapper 클래스 도입 (#21)
(DOCS) RULES.md 커밋 메시지 형식 규칙 추가 (#21)
(FEAT) 비밀번호 확인 로직 UseCase 미분리 버그 수정 (#21)
```
